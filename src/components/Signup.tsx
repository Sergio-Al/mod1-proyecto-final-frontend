...
function Signup() {
  ...
  const validatePassword = (password: string) => {
    ...
    if (!/[^A-Za-z0-9]/.test(password)) {
      newErrors.push('La contraseña debe contener al menos un caracter especial');
    }
    
    // Keep any confirm password error if it exists
    const confirmPasswordError = errors.find(error => error === 'Las contraseñas no coinciden');
    
    setErrors([...newErrors, ...(confirmPasswordError ? [confirmPasswordError] : [])]);
  ...
  const validateConfirmPassword = (confirmPassword: string) => {
    if (confirmPassword !== formData.password) {
      if (!errors.includes('Las contraseñas no coinciden')) {
        setErrors(prev => [...prev.filter(e => e !== 'Las contraseñas no coinciden'), 'Las contraseñas no coinciden']);
      }
      return false;
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        document.body.appendChild(createToast('¡Registro exitoso! Por favor inicia sesión.', 'success'));
        navigate('/login');
      } else {
        document.body.appendChild(createToast(data.message || 'Error en el registro', 'error'));
      }
    } catch (error) {
      document.body.appendChild(createToast('Error de conexión. Intente nuevamente.', 'error'));
      ...
      <div className="w-full max-w-md p-6">
        <form onSubmit={handleSubmit} className="card bg-base-200 shadow-xl">
          <div className="card-body">
            <h2 className="card-title text-2xl font-bold mb-4 text-center">Registro</h2>
